...
int dk2::CTextInput::fun_52AA10_configure(ButtonCfg *a2_cfg) {
    uint32_t v2_strId = a2_cfg->clickHandler_arg1;
    if (v2_strId) {
        unsigned __int8 *MbString = MyMbStringList_idx1091_getMbString(v2_strId);
        MBToUni_convert(MbString, this->str2, 128);
    } else {
        this->str2[0] = 0;
    }
    int f1A_inputLimit = (int) a2_cfg->clickHandler_arg2;
    if (f1A_inputLimit == -1) {
